...
```
├── part1a_read_audio.py          # Read and display audio file
├── part1b_short_term_energy.py   # Calculate short-term energy
├── part2a_frame_segmentation.py  # Divide into 20ms frames
├── part2b_zcr_calculation.py     # Calculate ZCR
├── part2c_classification.py     # Classify using ZCR and energy
...
### About PNG Image Files

**Important Note**: All PNG files in this project are generated from the attached audio file (`audio.flac`). These images represent the analysis and processing results of that specific audio file.

**For Users and Contributors**:
- If you replace the `audio.flac` file with your own audio file, all PNG images will be automatically regenerated based on your new audio file when you run the processing scripts again.
- Each audio file has its unique characteristics, so the generated images will differ for different audio files.
- To see results with your own audio file, simply replace the `audio.flac` file and re-run all the scripts.
...
This creates a professional Word document (`گزارش_پروژه_پردازش_سیگنال_صوتی.docx`) including:
- All project stages
